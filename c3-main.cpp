	...
	while (!viewer->wasStopped())
	{
		...
		if (!new_scan)
		{
			...
			pose = getPose(transform)
				...
				//declration of corrected scan
				PointCloudT::Ptr corrected(new PointCloudT);
			//transform filtered cloud
			pcl::transformPointCloud(*cloudFiltered, *corrected, *transform);
